...
public class Schedule_Frame extends JFrame {
	...
	Vector<PCB> block_list = new Vector<>();
	Vector<partiontable> partiontables_list= new Vector<>();
...
//				System.out.println("结束");
	...
	public PCB newpcb(JTextField nameArea,JTextField priorityJTextField,JTextField timeArea,JTextField memoryJTextField) {
		...
		System.out.println("新建一个PCB:"+"name:"+pcb.getName()+"priority:"+pcb.getPriority()+"state:"+pcb.getState()+"time:"+pcb.getTime()+"memory:"+pcb.getMemorysize());
		return pcb;
	}
	public void printpcb(PCB pcb) {
		System.out.println("name:"+pcb.getName()+"priority:"+pcb.getPriority()+"state:"+pcb.getState()+"time:"+pcb.getTime());
	}
		...
		if (!ready_list.isEmpty()&&ready_list.firstElement().getMemory_start_address()<0) {
			for (int i = 0; i < partiontables_list.size(); i++) {
				if ((partiontables_list.get(i).getState()==1)&&partiontables_list.get(i).getSize()>ready_list.firstElement().getMemorysize()) {
					...
					//剩下的可用分区
					partiontables_list.add(i+1,new partiontable(partiontables_list.get(i).getNumber(),partiontables_list.get(i).getSize()-ready_list.firstElement().getMemorysize(), partiontables_list.get(i).getStartaddress()+ready_list.firstElement().getMemorysize(), 1));
					//被分配的分区
...
//将所有前驱已经执行完毕的被阻塞的进程解救出来	
	...
	public void initialpartiontable() {
		partiontables_list.add(new partiontable(1,20,10,1));
		partiontables_list.add(new partiontable(2,10,31,1));
		partiontables_list.add(new partiontable(3,22,42,1));
		for (int i = 0; i < 65; i++) {
	...
	public void Backmemory(JList memoryJlist,Vector<PCB> runningList) {
		...
		for (int i = 0; i < partiontables_list.size(); i++) {
			for (int j = 0; j < partiontables_list.size(); j++) {
				partiontable itable = partiontables_list.get(i);
				partiontable jtable = partiontables_list.get(j);
